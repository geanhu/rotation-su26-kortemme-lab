...
def calculate_rmsd(
    ...
    # load structures
    reference_struc = parse_pdb(reference)
    mobile_struc = parse_pdb(mobile)
    ...
    if mode == 'all':
        ...
        return calc_rmsd(
            reference_struc['xyz'][:,1],
            mobile_struc['xyz'][:,1]
        ) 
    elif mode == 'backbone':
        return calc_rmsd(
            reference_struc['xyz'][:,:4],
            mobile_struc['xyz'][:,:4]
        )
...
'''
Code from Catherine Kuhn (Kortemme Lab)
'''
...
def main():
    ...
    # save output
    jsonpath = str(Path(args.input).parent) + f'/rmsd_{args.mode}_{Path(args.input).stem}_np.json'
